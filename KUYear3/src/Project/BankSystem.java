...
class BankSystem {
    ...
    public BankSystem() {
        this.users = new ArrayList<>();  // ใช้ ArrayList แทน List
        showMainMenu();
    }
    ...
    private void showCreateAccount() {
        ...
        submitButton.addActionListener(e -> {
            ...
            Database.saveUserToDatabase(name, pin, birthdate, gender, address, district, province, postalCode, balance, cardNumber);
    ...
    private void showLogin() {
        ...
        loginButton.addActionListener(e -> {
            ...
            User user = Database.getUserByCardNumberAndPin(cardNumber, pin);
            if (user != null) {
                frame.dispose();
                showDashboard(user);
            } else {
    ...
    private void showDashboard(User user) {
        ...
        depositButton.addActionListener(e -> {
            ...
            if (input != null) {
                ...
                user.getAccount().deposit(depositAmount);  // ฝากเงินในระบบ

                // อัปเดตยอดเงินในฐานข้อมูล
                Database.updateBalanceInDatabase(user.getCardNumber(), user.getAccount().getBalance());
            }
        });
        withdrawButton.addActionListener(e -> {
            ...
            if (input != null) {
                ...
                if (amount > user.getAccount().getBalance()) {
                    ...
                    // อัปเดตยอดเงินในฐานข้อมูลหลังจากถอนเงิน
                    Database.updateBalanceInDatabase(user.getCardNumber(), user.getAccount().getBalance());
                }
            }
        });
    ...
    // ฟังก์ชันสร้างหมายเลขบัตร
    private String generateCardNumber() {
        Random rand = new Random();
        StringBuilder cardNumber = new StringBuilder();
        for (int i = 0; i < 5; i++) {
            cardNumber.append(rand.nextInt(10)); // สุ่มตัวเลข 0-9
        }
